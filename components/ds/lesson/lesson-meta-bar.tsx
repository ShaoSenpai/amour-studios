"use client";

import Link from "next/link";
import { ChevronLeft } from "lucide-react";

export function LessonMetaBar({
  moduleTitle,
  moduleAccent,
  lessonOrder,
  lessonTotal,
  xpReward,
}: {
  moduleTitle: string;
  moduleAccent?: string;
  lessonOrder: number;
  lessonTotal: number;
  xpReward: number;
}) {
  return (
    <div className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-foreground/20 bg-foreground/[0.06] px-4 py-2.5 font-mono text-[12px] font-bold uppercase tracking-[1.5px]" style={{ fontFamily: "var(--font-body-legacy)" }}>
      <Link
        href="/dashboard"
        className="flex items-center gap-1 text-foreground/80 transition-colors hover:text-foreground"
        style={{ minHeight: 0 }}
      >
        <ChevronLeft size={14} />
        Dashboard
      </Link>
      <div className="flex items-center gap-3 text-foreground">
        <span
          className="italic text-[17px] font-normal"
          style={{ fontFamily: "var(--font-serif)", color: moduleAccent ?? "#FF6B1F" }}
        >
          {moduleTitle}
        </span>
        <span className="text-foreground/40">·</span>
        <span>LEÇON {String(lessonOrder).padStart(2, "0")} / {String(lessonTotal).padStart(2, "0")}</span>
        <span className="text-foreground/40">·</span>
        <span
          className="px-2 py-[2px]"
          style={{ background: "var(--state-done-bg)", color: "var(--state-done-fg)" }}
        >
          +{xpReward} XP
        </span>
      </div>
    </div>
  );
}
